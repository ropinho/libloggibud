...
loggibud::CVRPInstance loggibud::CVRPInstance::from_file(std::string filename) {
  const std::string kLoggibudSchemasDir =
      std::getenv("LOGGIBUD_SCHEMAS_DIR")
  ...
  if (file.is_open()) {
    auto filecontent = string(std::istreambuf_iterator<char>(file),
                              std::istreambuf_iterator<char>());
    file.close();

    std::string schema_path =
        kLoggibudSchemasDir + "/cvrp_instance.schema.json";
    ...
    if (!schema.validate(d)) {
      throw std::logic_error(
          "JSON file is not in accordance with CVRPInstance schema.");
    }
